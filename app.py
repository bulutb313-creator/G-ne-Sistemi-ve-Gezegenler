...
# --- RAG ZİNCİRİNİ BAŞLATAN FONKSİYON (Hata Kontrollü) ---
@st.cache_resource
def get_rag_chain():
    ...
    if not os.path.exists(file_path):
        st.error(f"KRİTİK HATA: '{file_path}' dosyası bulunamadı.")
        return None
    try:
        # 1. Veri İşleme
        full_text = "";
        ...
        # 3. RAG Zinciri Kurulumu (Analitik Prompt)
        llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0.2)
        base_retriever = vectorstore.as_retriever(search_kwargs={"k": 8})
        retriever = MultiQueryRetriever.from_llm(retriever=base_retriever, llm=llm)

        # PROMPT ŞABLONU (Zeki Robot)
...
def main():
    ...
    # API Anahtarını al ve kontrol et (Streamlit Secret'ı kullanır)
    # Streamlit Cloud'da GEMINI_API_KEY ortam değişkenini arar.
    if "GEMINI_API_KEY" not in os.environ:
        st.error("Lütfen Gemini API Anahtarınızı giriniz. Anahtar olmadan sistem çalışmaz.")
        # Bu kısım sadece lokalde çalışırken veya Secret'ı girmeyi unuttuğunuzda çıkar.
